 ...
 */
void
Trackball::trackball(float q[4], float p1x, float p1y, float p2x, float p2y)
{
 ...
 */
void
Trackball::axis_to_quat(float a[3], float phi, float q[4])
{
    vnormal(a);
    vcopy(a,q);
 ...
 */
void
Trackball::quat_mult(float q1[4], float q2[4], float dest[4])
{
    float tmp[4];
...
void
Trackball::add_quats(float q1[4], float q2[4], float dest[4])
{
    static int count=0;
 ...
 */
void
Trackball::build_rotmatrix(float m[4][4], float q[4])
{
...
void Trackball::matxmat(float out[4][4],float m0[4][4],float m1[4][4])
{
    float	r[4][4];
    int		i,j,k;
...
void Trackball::matident(float m[4][4])
{
	int 	i,j;
	for(i=0;i<4;i++) {
		for(j=0;j<4;j++) {
...
void Trackball::matmult(float *in,float *out,float mat[4][4])
{
        float tmp[3];
...
void Trackball::matinvert(float A[4][4])
{
/*
 *         a b c a b
 *         d e f d e
...
void Trackball::matcopy(float a[4][4],float b[4][4])
{
	memcpy(a,b,16*sizeof(float));
}

void Trackball::mattrans(float a[4][4])
{
	float	r[4][4];
	int	i,j;
	for(i=0;i<4;i++) {
...
void Trackball::matprint(float a[4][4],char *s)
{
	int i;
...
void Trackball::matinv4x4(float A[4][4])
{
	float	r[4][4];
	int	i,j;
...
float Trackball::matadjoint(float r[4][4],float _data[4][4])
{
	float outDet;
